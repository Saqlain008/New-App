...
const InvoicesView = (() => {
  ...
  function render(container) {
    ...
    var invoices = Store.Invoices.all().sort(function(a, b) { return b.createdAt - a.createdAt; });
    var wrap = Utils.el('div', { class: 'table-wrap' });
    if (!invoices.length) {
      wrap.appendChild(Utils.el('div', { class: 'table-empty' }, 'No invoices generated yet.'));
    } else {
      var paginated = Utils.paginate(invoices, currentPage, 15);
      currentPage = paginated.page;
      var table = Utils.el('table', {}, Utils.el('thead', {}, Utils.el('tr', {},
        ['Invoice #', 'Customer', 'Period', 'Total', 'Status', ''].map(function(h, i) { return Utils.el('th', { class: i === 3 ? 'num' : '' }, h); }))));
      ...
      paginated.items.forEach(function(inv) {
        tbody.appendChild(Utils.el('tr', {}, [
          ...
          Utils.el('td', { style: 'font-weight:600' }, custMap[inv.customerId] ? custMap[inv.customerId].name : '--'),
          ...
          Utils.el('td', {}, Utils.el('div', { class: 'row-actions' }, [
            Utils.el('button', { class: 'btn btn--sm', onclick: function() { viewInvoice(inv.id); } }, 'View'),
            Utils.el('button', { class: 'btn btn--sm btn--danger', onclick: function() {
              if (Utils.confirmDialog('Delete this invoice?')) { Store.Invoices.remove(inv.id); App.rerender(); }
            } }, 'Del')
          ]))
        ]));
      });
      ...
      wrap.appendChild(Utils.paginationControls(paginated, function(p) { currentPage = p; App.rerender(); }));
    }
  ...
  function openGenerateForm() {
    ...
    var weekStart = Utils.weekBounds(today).start;

    var body = Utils.el('form', { class: 'form-grid', id: 'invForm' }, [
      Utils.el('div', { class: 'field field--full' }, [
        Utils.el('label', {}, 'Customer *'),
        Utils.el('select', { name: 'customerId', required: 'required' },
          customers.map(function(c) { return Utils.el('option', { value: c.id }, c.name); }))
      ]),
    ...
    document.getElementById('invForm').addEventListener('submit', function(e) {
      ...
      if (data.periodStart > data.periodEnd) { Utils.toast('Period start must be before end.', 'error'); return; }
      var stmt = Billing.periodStatement(data.customerId, data.periodStart, data.periodEnd);
      var inv = Store.Invoices.add({
  ...
  function viewInvoice(invoiceId) {
    var inv = Store.Invoices.get(invoiceId);
    if (!inv) return;
    var cust = Store.Customers.get(inv.customerId);
    var settings = Store.getSettings();

    var body = Utils.el('div', {}, [
      Utils.el('div', { class: 'invoice', id: 'invoicePrintArea' }, [
        Utils.el('div', { class: 'invoice__top' }, [
          Utils.el('div', { style: 'display:flex;gap:12px;align-items:center' }, [
            Utils.el('div', { class: 'invoice__logo' }, 'M'),
        ...
        Utils.el('div', { class: 'invoice__parties' }, [
          Utils.el('div', {}, [Utils.el('h4', {}, 'Billed To'), Utils.el('div', { style: 'font-weight:700' }, cust ? cust.name : '--'), Utils.el('div', {}, cust ? cust.phone : ''), Utils.el('div', {}, cust ? (cust.address || cust.area || '') : '')]),
        ...
        Utils.el('div', { class: 'table-wrap', style: 'box-shadow:none' }, Utils.el('table', {}, [
          Utils.el('thead', {}, Utils.el('tr', {}, ['Description', 'Qty', 'Rate', 'Amount'].map(function(h, i) { return Utils.el('th', { class: i > 0 ? 'num' : '' }, h); }))),
      ...
      Utils.el('div', { class: 'form-actions' }, [
        Utils.el('button', { class: 'btn', onclick: function() { App.closeModal(); } }, 'Close'),
        Utils.el('button', { class: 'btn', onclick: function() { window.print(); } }, 'Print'),
        Utils.el('button', { class: 'btn btn--primary', onclick: function() { downloadInvoicePDF(inv, cust, settings); } }, 'Download PDF')
      ])
    ]);
  ...
  function downloadInvoicePDF(inv, cust, settings) {
    if (typeof window.jspdf === 'undefined') { Utils.toast('PDF library not loaded.', 'error'); return; }
    ...
    doc.text('Billed To: ' + (cust ? cust.name : '--') + ' (' + (cust ? cust.phone : '') + ')', left, y); y += 30;
  ...
  return { render: render, viewInvoice: viewInvoice };
})();